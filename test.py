...
MODEL_DIR = "/usr/share/ollama/.ollama/models/manifests/registry.ollama.ai/library"
FILE_DIR = 'files'
DATA_DIR = 'data'
...
if not st.session_state.logged_in:
    ...
    def initiate_chat():
        try:
            # Initialize the selected model if not already done
            ...
            # Update the model if a new model is selected
            if st.session_state.selected_model != selected_model:
                try:
                    ...
                    st.error("Failed to switch the model. Reverting to the previous model.")
                    return
                ...
                if files_in_selected_category:
                    ...
                    for selected_existing_file in selected_existing_files:
                        try:
                            ...
                            st.warning(f"Failed to load knowledge base: {selected_existing_file}. It will be skipped.")
                            continue
                    ...
                    if not selected_category:
                        ...
                        try:
                            ...
                            try:
                                ...
                                # Refresh the list of files in the selected category
                                files_in_selected_category = [
                                    f for f in os.listdir(category_dir) if f.endswith("_vectorstore")
                                ]
                                ...
                                if files_in_selected_category:
                                    st.sidebar.success(
                                        f"New files have been processed and added to the knowledge base."
                                    )

                                    # Dynamically update the dropdown with new files
                                    selected_existing_files = st.sidebar.multiselect(
                                        "Select Knowledge Base:",
                                        files_in_selected_category,
                                        default=[f for f in files_in_selected_category if f in st.session_state.processed_files],
                                    )

                            except Exception:
                                st.warning("Some files could not be processed. Please check their format and try again.")

                        except Exception as e:
                            st.error(f"An error occurred while uploading files. Please try again.")
            ...
            #                     os.remove(file_path)
            #             except Exception:
            #                 st.warning(f"Failed to delete file: {file_name}.")
            ...
            if len(st.session_state.retrievers) > 1:
                try:
                    ...
                    st.warning("Failed to create an ensemble retriever. Using individual retrievers instead.")
                    ensemble_retriever = None
            else:
                ensemble_retriever = st.session_state.retrievers[0] if st.session_state.retrievers else None
            ...
            st.error(f"Error:{e}")
            # st.success("QA chain initialized for multi-file chat.")
        ...
        with input_container:
            ...
            # Add the Stop button next to the user input field
            ...
            if user_input:
                if user_input.strip():
                    # Reset stop signal for new query
                    st.session_state.stop_processing = False
                    user_message = {"role": "user", "message": user_input}
                    st.session_state.chat_history.append(user_message)

                    # Generate assistant's response with stop logic
                    try:
                        ...
                        if str(e) == "Processing stopped by user.":
                            st.toast("Processing stopped.")
                        else:
                            st.toast("An error occurred during processing.")
                    finally:
                        # Reset the stop flag after processing
                        st.session_state.stop_processing = False
