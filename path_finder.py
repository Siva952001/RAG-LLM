...
if not st.session_state.logged_in:
    ...
    # Initialize the selected model and LLM
    ...
    selected_model = st.selectbox("Select Model", available_models, index=available_models.index(st.session_state.selected_model) if st.session_state.selected_model in available_models else 0)
    ...
    # Dropdown for selecting the category before uploading files
    category_options = get_categories_from_db()

    # Append a special option for creating a new category (for admin users only)
    if st.session_state.is_admin:
        category_options.append("+ Create New Category")

    # Add a dropdown for selecting the category before uploading files
    selected_category = st.sidebar.selectbox(
        "Select Category", 
        category_options, 
        help="Choose the category for file upload"
    )

    # Store the selected category in session state
    st.session_state.selected_category = selected_category

    # Logic for handling "Create New Category" input field and "OK" button
    if selected_category == "+ Create New Category" and st.session_state.is_admin:
        st.sidebar.markdown("### Create a New Category")
        ...
        if st.sidebar.button("OK"):
            if new_category:
                ...
                existing_category = db['knowledge_base'].find_one({"category_name": new_category})
        ...
        if selected_category and selected_category != "+ Create New Category":
            ...
            # Check for existing vector stores in the selected category
            existing_files = []
            for file in os.listdir(category_dir):
                if file.endswith("_vectorstore"):  # Adjust based on your vector store naming convention
                    existing_files.append(file.replace("_vectorstore", ""))  # Get file names without vector store suffix

            # If there are existing files, allow the user to choose one to chat with
            if existing_files:
                selected_existing_file = st.sidebar.selectbox(
                    "Select a previously uploaded file to chat with:",
                    ["Select a file"] + existing_files,
                    help="Choose a file to interact with."
                )

                if selected_existing_file != "Select a file":
                    vectorstore_path = os.path.join(category_dir, f"{selected_existing_file}_vectorstore")
                    ...
                    st.sidebar.success(f"You can now chat with '{selected_existing_file}'.")
            ...
            if uploaded_files:
                for uploaded_file in uploaded_files:
                    ...
                    vectorstore_path = os.path.join(category_dir, f"{uploaded_file.name}_vectorstore")

                    if file_key in st.session_state.processed_files:
                        st.sidebar.warning(f"⚠️ File '{uploaded_file.name}' already exists. You can ask questions.")
                    elif os.path.exists(vectorstore_path):
                        st.sidebar.warning(f"⚠️ The vector store for '{uploaded_file.name}' is already present. You can continue your chat.")
                        st.session_state.vectorstore = Chroma(persist_directory=vectorstore_path, embedding_function=st.session_state.embedding_model)
                        st.session_state.retriever = st.session_state.vectorstore.as_retriever()
                        ...
                        try:
                            with st.spinner("🔍 Analyzing your document..."):
                                ...
                                st.sidebar.info(f"Chunk size: {chunk_size}, Overlap: {chunk_overlap}")
                                ...
                                if not splits:
                                    st.sidebar.warning(f"No content to process for '{uploaded_file.name}'.")
                                    continue
                                documents = [Document(page_content=split) for split in splits]
                                if not documents:
                                    st.sidebar.warning(f"No valid documents created for '{uploaded_file.name}'.")
                                    continue
