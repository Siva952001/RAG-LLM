...
if not st.session_state.logged_in:
    ...
    def initiate_chat():
        ...
        selected_category = st.sidebar.multiselect("Select Domain/Project", category_options)
        ...
        if selected_category:
            ...
            if files_in_selected_category:
                selected_existing_file = st.sidebar.selectbox(
                    ...
                    ["Select a file"] + files_in_selected_category,
                )

                # Set up vectorstore and retriever if a file is selected
                if selected_existing_file != "Select a file":
                    selected_category_dir = os.path.join(DATA_DIR, selected_category[0])
                    vectorstore_path = os.path.join(selected_category_dir, selected_existing_file)
        ...
        if uploaded_files:
            if not selected_category:
                ...
                try:
                    ...
                    for category in selected_category:
                        category_dir = os.path.join(DATA_DIR, category)
                        existing_file_names.update(os.listdir(category_dir))
                    ...
                    if failed_uploads:
                        st.toast(
                            f"Uploaded {len(uploaded_files) - len(failed_uploads)} files to '{', '.join(selected_category)}' category. {len(failed_uploads)} files were not uploaded due to duplicates."
                        )
                    else:
                        st.toast(
                            f"Uploaded {len(uploaded_files)} files to '{', '.join(selected_category)}' category."
                        )
        ...
        def render_chat_history():
            chat_history_html = "<div class='chat-history' style='height: 450px; overflow-y: auto; padding-right: 10px;background-color: #f5f5f5;'>"
